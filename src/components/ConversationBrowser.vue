...
import { parseFileDragPayload } from '../utils/dragDropPayload'
...
const handleMerge = async () => {
  ...
  if (
    !confirm(
      `确定要将 ${selectedFilesToMerge.value.length - 1} 个对话合并到 "${selectedFilesToMerge.value[0]!.name}" 吗？\n被合并的源文件将被删除。`,
    )
  ) {
    return
  }
  ...
  try {
    ...
    await mergeConversations(
      ...
      sources.map((f) => ({
        handle: f.handle,
        parentHandle: f.parentHandle || chatsDirHandle.value!,
      })),
    )
...
const focusSelectedItem = async () => {
  ...
  const tryFocus = async () => {
    ...
    const items = Array.from(
      container.querySelectorAll('.item-content[tabindex="0"]'),
    ) as HTMLElement[]
...
const handleFileClick = (event: {
  ...
  if (isSelectionMode.value) {
    ...
    if (index !== -1) {
      ...
      // Select
      selectedFilesToMerge.value.push({
        ...event.entry,
...
const handleDeleteEntry = async (
  {
    entry,
    parentHandle,
  }: { entry: FileSystemEntry; parentHandle: FileSystemDirectoryHandle | null },
  force = false,
) => {
...
const handleMoveEntry = async ({
  ...
  sourceName: string
  sourceParentHandle: FileSystemDirectoryHandle | null
...
const handleRootDrop = async (event: DragEvent) => {
  ...
  const data = parseFileDragPayload(event.dataTransfer)
  if (data) {
...
<template>
  <div class="conversation-browser">
    <div class="header">
      ...
      <div class="header-actions">
        ...
        <button
          ...
          @click="handleMerge"
          :disabled="selectedFilesToMerge.length < 2"
          title="合并"
          style="font-size: 1rem"
        >
          合并 ({{ selectedFilesToMerge.length }})
        </button>
        <button @click="handleCreateDirectory" :disabled="!rootHandle" title="新建文件夹">
          📁+
        </button>
      </div>
    </div>
    ...
    >
      <div v-if="!rootHandle" class="placeholder">请选择一个目录来开始。</div>
      <div v-else-if="isLoading" class="placeholder">正在加载文件...</div>
      <div v-else-if="fileTree.length === 0" class="placeholder">"chats" 目录为空或不存在。</div>
      <FileTreeItem
        v-else
        :entries="fileTree"
