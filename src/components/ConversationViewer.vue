  ...
  <div class="conversation-viewer">
    <div class="viewer-header">
      ...
      <div style="display: flex; gap: 10px; align-items: center">
        ...
        <div class="action-group">
          <button @click="toggleSortingMode" :class="{ 'active-btn': isSortingMode }">
            {{ isSortingMode ? '退出排序' : '排序' }}
          </button>
          <button @click="handleSelectAll" :disabled="isAllSelected">全选</button>
          <button @click="handleClearSelection" :disabled="selectionState.length === 0">
            清空
          </button>
        </div>
        ...
        <div class="action-group">
          <button
            @click="handleCheckFormatting"
            :disabled="selectionState.length === 0"
            title="检查选中内容是否有Markdown格式问题"
          >
            修复格式
          </button>
          <button
        ...
        <button @click="handleGenerateImageClick" :disabled="selectionState.length === 0">
          分享
        </button>
      </div>
    </div>
    ...
    <div class="viewer-content">
      ...
      <div
        v-else
        class="chat-log"
        :class="{ 'is-dragging': !!draggedTurnState || isSortingMode }"
      ...
      >
        ...
        <div
          ...
          :class="{
            active:
              dropTargetIndex === conversation.length && !isDropRedundant(conversation.length),
          }"
        ></div>
      </div>
          ...
          <div v-for="(item, idx) in fixPreviewData" :key="idx" class="fix-item">
            ...
            <div class="diff-container raw-mode" v-else>
              <div class="diff-box">
                ...
                <pre
                  class="code-diff"
                ><code><template v-for="(part, pIdx) in item.diffs" :key="pIdx"><span :class="'diff-' + part.type">{{ part.value }}</span></template></code></pre>
              </div>
            </div>
          </div>
        </div>
...
.drop-zone-end.active {
  background-color: rgba(
    ...
    0.1
  ); /* Assuming variable exists, or just light blue */
  background-color: #e7f5ff;
