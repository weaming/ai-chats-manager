...
import { getSubDirectoryHandle, moveFile, directoryExists } from '../utils/fileSystemUtils'
...
export function useFileSystem() {
  ...
  const listDirectory = async (
    ...
    const targetHandle =
      dirHandle || (await getSubDirectoryHandle(rootHandle.value, 'chats', { create: true }))
  ...
  const createDirectory = async (dirName: string, parentDirHandle?: FileSystemDirectoryHandle) => {
    ...
    const parentHandle =
      parentDirHandle || (await getSubDirectoryHandle(rootHandle.value, 'chats', { create: true }))

    try {
      if (await directoryExists(parentHandle, finalDirName)) {
        throw new Error(`文件夹 "${finalDirName}" 已存在。`)
      }
      ...
      throw error instanceof Error ? error : new Error(`创建目录 "${finalDirName}" 失败。`)
    }
  }
  ...
  const renameDirectory = async (
    ...
    try {
      if (await directoryExists(parentDirHandle, finalNewName)) {
        throw new Error(`目标文件夹 "${finalNewName}" 已存在。`)
      }
      ...
      throw error instanceof Error
        ? error
        : new Error(`重命名目录 "${dirHandle.name}" 为 "${finalNewName}" 失败。`)
    }
  }
  ...
  const moveConversation = async (
    ...
    try {
      ...
      throw error instanceof Error ? error : new Error(`移动文件 "${sourceName}" 失败。`)
    }
  }
